...
public class PasswordResetRequestSettings {
    ...
    @Pattern(regexp = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#&()–[{}]:;',?/*~$^+=<>]).{6,50}$",
        message = "Current password must contain lower and upper case letters, "
    ...
    @Pattern(regexp = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#&()–[{}]:;',?/*~$^+=<>]).{6,50}$",
        message = "Password confirmation must contain lower and upper case letters, "
                + "at least one number, as well as one special character. "
                + "Password must be at least 6 characters long.")
    @Size(min = 6, max = 50, message = "Password confirmation must be from 6 to 50 characters long.")
    private String passwordConfirmaiton;
